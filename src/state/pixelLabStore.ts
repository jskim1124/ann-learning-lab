...
import { createPixelProjection, type PixelProjection } from "../visualization/pixelLatentMap";
...
export class PixelLabStore {
  ...
  private fresh(task: PixelTaskName): PixelLabState {
    ...
    return { task, data, model, drawing: sampleForClass(task, 0), selectedLabel: 0, learningRate: .12, history: [{ epoch: 0, loss: evaluatePixelModel(model, data).loss }], understandStep: 1, highlightRevealed: false, quizPassed: false, projection: createPixelProjection(data), view: "decision" };
  }
  ...
  paint(index: number, value = 1): void { if (index < 0 || index >= PIXEL_INPUTS) return; const drawing = [...this.state.drawing]; drawing[index] = value; this.state = { ...this.state, drawing }; this.emit(); }
  clear(): void { this.state = { ...this.state, drawing: emptyDrawing(this.state.task) }; this.emit(); }
  loadSample(label: number, variation = 0): void { this.state = { ...this.state, drawing: sampleForClass(this.state.task, label, variation), selectedLabel: label }; this.emit(); }
