...
export function networkGraphMarkup(model: NetworkModel, activations: number[]): string {
  ...
  hiddenYs.forEach((y, h) => {
    ...
    lines.push(weightLine(inputX, 76, hiddenX, y, pair[0], `가로값 → 규칙 ${h + 1}`));
    lines.push(weightLine(inputX, 154, hiddenX, y, pair[1], `세로값 → 규칙 ${h + 1}`));
    lines.push(weightLine(hiddenX, y, outputX, 115, model.parameters.hiddenOutput[h] ?? 0, `규칙 ${h + 1} → 결과`));
    const activation = activations[h] ?? 0;
    const intensity = Math.min(1, Math.abs(activation));
    nodes.push(`<g class="network-node" data-kind="hidden"><circle cx="${hiddenX}" cy="${y}" r="15" style="fill:rgba(35,103,213,${0.12 + intensity * 0.55})"/><text x="${hiddenX}" y="${y + 4}">${activation.toFixed(2)}</text><title>규칙 찾기 칸 ${h + 1}: ${activation.toFixed(5)}</title></g>`);
  });
