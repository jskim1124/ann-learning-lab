...
export function pixelNetworkGraphMarkup(model: PixelModel, labels: readonly string[], hiddenValues: number[]): string {
  ...
  const hiddenNodes = hiddenY.map((y, h) => { const value = hiddenValues[h] ?? 0; const alpha = (.12 + Math.abs(value) * .32).toFixed(2); return `<g><circle cx="165" cy="${y}" r="12" fill="rgba(116,70,245,${alpha})" stroke="#7446f5"/><text x="165" y="${y + 3}" text-anchor="middle">${h + 1}</text></g>`; }).join("");
  const outputNodes = outputY.map((y, c) => `<g><circle cx="276" cy="${y}" r="14" fill="#fff" stroke="${COLORS[c % COLORS.length]}" stroke-width="3"/><text x="276" y="${y + 4}" text-anchor="middle" fill="${COLORS[c % COLORS.length]}" font-weight="800">${escape(labels[c] ?? String(c + 1))}</text></g>`).join("");
