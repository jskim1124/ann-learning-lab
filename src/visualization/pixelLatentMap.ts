...
export function drawPixelNeuronMovement(canvas: HTMLCanvasElement, before: PixelModel, after: PixelModel, data: PixelExample[], focusPixels: number[], projection: PixelProjection, focusLabel = "이 그림", neuron = 0): void {
  ...
  const start = toCanvas(nearest(oldPlane)); const end = toCanvas(nearest(newPlane)); context.strokeStyle = "#d08700"; context.fillStyle = "#d08700"; context.lineWidth = 4; context.beginPath(); context.moveTo(start.x, start.y); context.lineTo(end.x, end.y); context.stroke(); const angle = Math.atan2(end.y - start.y, end.x - start.x); context.beginPath(); context.moveTo(end.x, end.y); context.lineTo(end.x - 12 * Math.cos(angle - .48), end.y - 12 * Math.sin(angle - .48)); context.lineTo(end.x - 12 * Math.cos(angle + .48), end.y - 12 * Math.sin(angle + .48)); context.closePath(); context.fill();
