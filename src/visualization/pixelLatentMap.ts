...
export function drawPixelNeuronMovement(canvas: HTMLCanvasElement, before: PixelModel, after: PixelModel, data: PixelExample[], focusPixels: number[], projection: PixelProjection): void {
  const context = canvas.getContext("2d"); if (!context) return;
  const ratio = window.devicePixelRatio || 1; const rect = canvas.getBoundingClientRect(); const width = Math.max(520, Math.round(rect.width || 700)); const height = Math.max(300, Math.round(rect.height || 420));
  if (canvas.width !== width * ratio || canvas.height !== height * ratio) { canvas.width = width * ratio; canvas.height = height * ratio; }
  ...
  const focus = toCanvas(projectPixels(projection, focusPixels)); context.beginPath(); context.arc(focus.x, focus.y, 9, 0, Math.PI * 2); context.fillStyle = "rgba(255,255,255,.9)"; context.fill(); context.strokeStyle = "#111722"; context.lineWidth = 3; context.stroke();
