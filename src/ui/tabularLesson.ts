...
export class TabularLesson {
  ...
  render(draft:CustomDatasetDraft):void{
    ...
    else{line(`같은 점의 합 ${fmt(pixelHiddenLineValue(before,point.pixels,0))} → ${fmt(pixelHiddenLineValue(after,point.pixels,0))}`);line(`정답 점수 ${(forwardPixels(before,point.pixels).probabilities[row.label]!*100).toFixed(2)}% → ${(forwardPixels(after,point.pixels).probabilities[row.label]!*100).toFixed(2)}%`);line("틀린 정도를 줄이는 방향으로 연결값을 고칩니다. 매번 같은 방향으로 움직이지는 않습니다.");if(this.frame===30)line(`${this.prediction==="up"?"예상대로":"예상과 달리"} 합이 커졌습니다. 점은 그대로이고 연결값과 기준선이 바뀌었습니다.`);}
