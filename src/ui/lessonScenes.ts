...
export function renderOutputScene(root:HTMLElement,focus:number[],stage:number,hidden:number):SceneQuiz {
  ...
  el(root,"flText").textContent=["앞 예제와 같은 계산 방법입니다. 두 번째 뉴런을 켜면 다른 기준을 더 볼 수 있습니다.","이 예제는 가장 간단한 변환을 씁니다. 합이 −0.2면 신호 0, 합이 0.6이면 신호 0.6입니다.","출력 A·B·C는 각 뉴런 신호에 연결값을 곱해 더합니다. 가장 큰 점수의 답을 고릅니다.","음수도 가능한 출력값을 같은 변환표로 양수로 바꿉니다. 그 뒤 전체 합으로 나누고 100을 곱합니다. 아래는 실제 계산값입니다.","은닉 뉴런은 판단에 쓸 기준을 만듭니다. 출력은 클래스마다 하나씩입니다. 은닉 뉴런 1개여도 A·B·C 점수 3개를 계산하지만, 잘 구분할 수 있다는 보장은 아닙니다."][stage]!;
  let html="";
...
export function renderMovementScene(root:HTMLElement,s:ImageState,toy:boolean,frame:number,bias:ReturnType<typeof biasDirectionExample>,movement:ReturnType<typeof featureMovementExample>,prediction:string|null):SceneQuiz {
  ...
  if(toy){const b=after.hiddenBias[0]!,h=r.hidden[0]!;
    el(root,"flCalculation").innerHTML=`<span>① 뉴런의 합: 0.2 × 1 + 0.2 × 0.5 + <b>${n(b)}</b> = <b>${n(.3+b)}</b></span><span>② 신호: 음수면 0, 양수면 그대로 → <b>${n(h)}</b></span><span>③ 출력 A: 1 − ${n(h)} = <b>${n(r.logits[0]!)}</b>　B: ${n(h)} × 1 = <b>${n(r.logits[1]!)}</b></span><div class="movement-scores"><span>A <b>${n(r.logits[0]!)}</b></span><span>B <b>${n(r.logits[1]!)}</b></span><strong>${Math.abs(r.logits[0]!-r.logits[1]!)<1e-8?"같은 점수":r.logits[0]!>r.logits[1]! ? "지금 예상 A":"지금 예상 B"}</strong></div><span>더해주는 값이 0.1 커지면 신호는 0.1 커지고, A는 0.1 작아지고 B는 0.1 커집니다.</span>`;
    el(root,"flSelected").innerHTML=`<div class="boundary-position"><b>가로 점선(세로 0.2)에서</b><span>A와 B가 같으려면 신호 = 0.5</span><span>가로 위치 + 0.1 + ${n(b)} = 0.5 → 경계의 가로 위치 = <b>${n(.4-b)}</b></span><span>더해주는 값 ↑ → 같은 합을 유지할 가로 위치 ↓ → 경계가 왼쪽으로</span></div>`;
    el(root,"flVisualNote").textContent=frame>0&&prediction?(prediction==="up"?"맞아요. 더해주는 값을 늘리니 B 영역이 이 점 쪽으로 넓어집니다.":"빼면 B 점수가 줄어듭니다. 늘렸을 때의 점수와 영역을 확인하세요."):"보라 선 = 뉴런의 합 0 · 검은 선 = A와 B 점수가 같은 곳 · 파란 화살표 = 경계 이동";
