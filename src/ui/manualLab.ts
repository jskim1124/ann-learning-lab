...
export class ManualLabWorkspace {
  ...
  constructor(private getSource:()=>ManualSource) {
    this.dialog=document.createElement('dialog');this.dialog.className='manual-lab';this.dialog.setAttribute('aria-labelledby','manualTitle');
    this.dialog.innerHTML=`<header class="manual-header"><div><span>원래 모델을 바꾸지 않는 실험실</span><h2 id="manualTitle">내가 모델이 된다면?</h2></div><button data-manual="close" aria-label="실험실 닫고 연습으로 돌아가기">연습으로 돌아가기 ×</button></header>
      ...
      <div class="manual-layout"><section class="manual-map-panel"><div class="manual-map-tools"><button data-gesture="point">점 살펴보기</button><button data-gesture="line">선 잡고 옮기기</button><label><input id="manualBefore" type="checkbox"> 고치기 전 선</label></div><canvas id="manualMap" tabindex="0" aria-label="직접 고치는 뉴런 선과 최종 경계. 점을 선택하거나 선 옮기기 모드로 드래그하세요."></canvas><p id="manualPoint" class="manual-point"></p><div class="manual-score"><strong id="manualScore"></strong><span id="manualChange"></span><span id="manualBest"></span></div><p class="manual-legend">색 선: 뉴런 합이 0인 곳 · 검은 선: 가장 높은 답 점수가 바뀌는 곳</p></section>
    ...
    this.dialog.addEventListener('change',e=>{const input=e.target as HTMLInputElement;if(input.id==='manualBefore'){this.showBefore=input.checked;this.draw();}if(input.id==='manualOutput'){this.lab.output=Number(input.value);this.predicting=false;this.render();}if(input.id==='manualMute'){this.lab.muted=input.checked?this.lab.selected:null;this.predicting=false;this.render();}});
  ...
  private click(event:MouseEvent):void {
    ...
    if(d.guess){const p=l.biasPreview(),truth=Math.abs(p.after-p.before)<1e-9?'same':p.after>p.before?'up':'down';if(d.guess!==truth){this.feedback='오답이에요. 뉴런이 넘긴 수를 이 답에 더하는지 빼는지 살펴보세요.';this.feedbackKind='wrong';}else{l.edit('bias',l.model.hiddenBias[l.selected]!+.25);this.predicting=false;this.feedback=`맞았어요. ${l.source.classes[l.output]} 점수 ${num(p.before)} → ${num(p.after)}. 선과 예상도 함께 확인하세요.`;this.feedbackKind='correct';}}
  ...
  render():void {
    ...
    this.el('manualGoal').textContent=titles[this.mission];
    ...
    const outputChoice=`<label class="manual-output-choice">고칠 답 <select id="manualOutput">${source.classes.slice(1).map((c,i)=>`<option value="${i+1}" ${i+1===l.output?'selected':''}>${esc(c)}</option>`).join('')}</select></label>`;
    ...
    if(rebuild){
    if(this.tab==='line')this.el('manualControls').innerHTML=`<p class="manual-instruction">점을 그대로 두고 뉴런 ${h+1}의 선을 바꿔 보세요.</p>${slider('마지막에 더할 값', 'bias',m.hiddenBias[h]!)}<label class="manual-direction">값이 커지는 방향<select data-parameter="direction" aria-label="뉴런 값이 커지는 방향">${directions.map((s,i)=>`<option value="${i}" ${i===direction?'selected':''}>${s}</option>`).join('')}</select></label><small>위의 ‘선 잡고 옮기기’를 고르면 그래프에서도 움직일 수 있어요.</small>`;
    else if(this.tab==='output')this.el('manualControls').innerHTML=`${outputChoice}${slider(`뉴런 ${h+1}의 수에 곱할 값`,'connection',m.hiddenOutput[l.output]![h]!,.5)}${slider('답에 마지막으로 더할 값','outputBias',m.outputBias[l.output]!)}<label><input id="manualMute" type="checkbox" ${l.muted!==null?'checked':''}> 뉴런 ${h+1} 없이 비교</label>`;
    else{
    ...
    const mute=this.dialog.querySelector<HTMLInputElement>('#manualMute');if(mute)mute.checked=l.muted!==null;
    this.dialog.querySelector('.manual-output-score')?.remove();
    if(this.tab==='output'){const r=l.calculation;this.el('manualControls').insertAdjacentHTML('beforeend',`<p class="manual-output-score">${esc(source.classes[l.output]!)} 점수: ${r.hidden.map((v,i)=>num(v*l.effective.hiddenOutput[l.output]![i]!)).map(v=>`(${v})`).join(' + ')} + (${num(m.outputBias[l.output]!)}) = <b>${num(r.logits[l.output]!)}</b><small>${esc(source.classes[0]!)}는 비교 기준 0. 가장 높은 답 점수가 바뀌는 곳이 검은 경계예요.</small></p>`);}
