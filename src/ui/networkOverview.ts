...
export function networkOverview(model: PixelModel, input: number[], labels: readonly string[], selected: boolean, neuron: number, output: number, options: {classColors?:readonly string[];onlyOutput?:number;showValues?:boolean} = {}): string {
  ...
  const colors = options.classColors ?? (reference ? ['#1f6bd6', '#f17605'] : CLASS_COLORS);
  const visibleOutputs=result.probabilities.map((value,index)=>({value,index})).filter(o=>options.onlyOutput===undefined||o.index===options.onlyOutput);
  const columns = model.hiddenUnits > 12 ? 3 : model.hiddenUnits > 4 ? 2 : 1;
  const rows = Math.ceil(model.hiddenUnits / columns);
  const height = Math.max(152, rows * 30, visibleOutputs.length * 36);
  ...
  const outputs = visibleOutputs.map((o,i) => ({ ...o, y: position(i, visibleOutputs.length) }));
  ...
  const lines = hidden.map((h, i) => `<path data-input-wire="${i}" d="${path(35, height / 2, h.x, h.y)}" class="${i === neuron ? 'is-route' : ''}" style="--wire-color:${NEURON_COLORS[i % NEURON_COLORS.length]}"/>`).join('') + hidden.map((h, i) => outputs.map(o => `<path data-output-wire="${i}-${o.index}" d="${path(h.x, h.y, 280, o.y)}" class="${i === neuron ? 'is-route' : ''}" style="--wire-color:${NEURON_COLORS[i % NEURON_COLORS.length]}"/>`).join('')).join('');
  return `<div class="network-overview" aria-label="입력, 은닉 뉴런의 반응, 답별 예상">
    <div class="network-headings"><span>입력</span><span>은닉 뉴런</span><span>${options.onlyOutput===undefined?'예상':'고른 답'}</span></div>
    <div class="network-stage" style="height:${height}px">
      ...
      ${options.showValues?`<span class="network-route-value" style="left:${hidden[neuron]!.x/4}%;top:${hidden[neuron]!.y/height*100}%;--node-color:${NEURON_COLORS[neuron%NEURON_COLORS.length]}" aria-label="뉴런 ${neuron+1}이 넘긴 수">${result.hidden[neuron]!.toFixed(2)}</span><span class="network-wire-value" style="left:57%;top:${(hidden[neuron]!.y+(outputs.find(o=>o.index===output)?.y??height/2))/2/height*100}%">× ${model.hiddenOutput[output]![neuron]!.toFixed(2)}</span>`:''}
      ...
      ${hidden.map((h, i) => `<button type="button" class="response-node" data-network-neuron="${i}" aria-label="뉴런 ${i + 1} 경로 보기${selected ? `, 반응 ${h.value.toFixed(2)}` : ''}" aria-pressed="${i === neuron}" style="left:${h.x / 4}%;top:${h.y / height * 100}%;--node-color:${NEURON_COLORS[i % NEURON_COLORS.length]};--response-ink:${selected ? 12 + responseInk(h.value, model.activation) * 68 : 0}%"><span>${i + 1}</span>${selected ? `<small aria-hidden="true">${h.value > 0 ? '+' : h.value < 0 ? '−' : '·'}</small>` : ''}</button>`).join('')}
      ${outputs.map(o => {const i=o.index;return `<button type="button" class="response-answer${selected && winners.includes(i) ? ' is-leading' : ''}" data-network-output="${i}" aria-label="${esc(labels[i] ?? String(i))} 출력 보기${selected ? `, 예상 ${(o.value * 100).toFixed(2)}%` : ''}" aria-pressed="${i === output}" style="left:82%;top:${o.y / height * 100}%;--class-color:${colors[i % colors.length]}"><span>${esc(labels[i] ?? String(i))}</span><i class="answer-track" aria-hidden="true"><i style="width:${selected ? o.value * 100 : 0}%"></i></i></button>`;}).join('')}
    </div>
    ...
    <p class="network-key network-change" aria-live="polite">${options.showValues?'고른 뉴런의 수 → 연결값을 곱해 답에 보태요.':'뉴런을 누르면 그 길의 값만 보여요.'}</p>
  </div>`;
}
