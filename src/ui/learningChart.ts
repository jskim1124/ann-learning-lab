...
export function openChartHelp():void {
  if(!help||!help.isConnected){
    ...
    help.innerHTML='<header><h2>오차와 정답률은 달라요</h2><button data-close aria-label="그래프 설명 닫기">×</button></header><nav><button data-chart-topic="metrics" aria-pressed="true">오차와 정답률</button><button data-chart-topic="landscape" aria-pressed="false">전역·지역 최적점</button></nav><section data-chart-page="metrics"><p>정답 B를 51%로 골라도, 99%로 골라도 둘 다 맞힌 답입니다. 정답률은 같지만 정답에 더 높은 점수를 준 쪽의 오차가 작습니다.</p><div class="loss-comparison"><span>B 51%<b>정답률 100% · 오차 0.67</b></span><span>B 99%<b>정답률 100% · 오차 0.01</b></span></div><p><b>연습 그래프</b>의 가로축은 학습 횟수입니다. 선이 멈췄다는 이유만으로 가장 좋은 모델을 찾았다고 할 수 없습니다.</p></section><section data-chart-page="landscape" hidden><h3>최적점은 ‘오차 지형’으로 따로 볼까요?</h3><p>아래는 원리 설명용 모형입니다. 실제 내 모델의 학습 기록이 아닙니다. 가로축은 학습 횟수가 아니라 <b>바꾸는 값 하나</b>입니다.</p><canvas width="650" height="240" aria-label="서로 다른 높이의 두 골짜기가 있는 예제 오차 지형"></canvas><div class="landscape-actions"><button data-descent="-1.5">왼쪽에서 출발 ▶</button><button data-descent="1.5">오른쪽에서 출발 ▶</button></div><p data-landscape-status role="status">오차가 낮아지는 쪽으로 내려가 보세요.</p><p class="chart-caveat">주변보다 낮은 곳은 지역 최적, 전체에서 가장 낮은 곳은 전역 최적입니다. 실제 신경망에는 바꾸는 값이 많이 있습니다. 정답률이 높아도 새 자료에서 잘 맞히는지는 따로 확인해야 합니다.</p></section>';
