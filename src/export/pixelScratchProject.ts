...
export function createPixelScratchProjectFiles(model: PixelModel, labels: readonly string[], pixels: readonly number[] = []): Record<string, Uint8Array> {
  ...
  const mathop = (operator: string, value: Expr): Expr => (parent) => { const blockId = id("math"); blocks[blockId] = { opcode: "operator_mathop", next: null, parent, inputs: {}, fields: { OPERATOR: [operator, null] }, shadow: false, topLevel: false }; blocks[blockId].inputs.NUM = value(blockId); return [2, blockId]; };
  ...
  const activation = (value: Expr): Expr => model.activation === "relu" ? div(add(value, mathop("abs", value)), num(2)) : model.activation === "sigmoid" ? div(num(1), add(num(1), exp(mul(num(-1), value)))) : tanh(value);
  ...
  const addHidden = id("addhidden"); blocks[addHidden] = { opcode: "data_addtolist", next: null, parent: null, inputs: {}, fields: { LIST: ["숨은 값", listIds.hidden] }, shadow: false, topLevel: false }; blocks[addHidden].inputs.ITEM = activation(variable("합"))(addHidden);
