...
export const PRESETS: Record<PresetName, DatasetPreset> = {
  ...
  xor: {
    title: "승부차기에서 골이 들어갈까?",
    description: "키커와 골키퍼가 고른 방향으로 골과 막힘의 네 경우를 비교합니다.",
    story: "체육대회 승부차기를 앞두고 민준이네 반이 운동장에서 연습하고 있습니다. 키커는 왼쪽이나 오른쪽으로 차고, 골키퍼도 한쪽을 골라 몸을 던집니다.",
    motivation: "먼저 공의 높이와 세기는 같다고 단순하게 정해 봅시다. 두 사람이 같은 방향을 고르면 막히고, 다른 방향을 고르면 골입니다. 이 네 경우를 모델이 구별할 수 있을까요?",
    question: "은닉 뉴런 두 개가 대각선으로 떨어진 두 골을 어떻게 한 영역으로 묶는지 찾아봅시다.",
    axes: ["키커의 방향: 왼쪽 → 오른쪽", "골키퍼의 방향: 왼쪽 → 오른쪽"], classes: ["막힘", "골"], difficulty: "XOR 원리", sourceNote: "실제 승부차기 결과에는 공의 높이·속도·정확도도 영향을 줍니다. 여기서는 XOR 모양을 알아보기 위해 방향만 남긴 연습 규칙을 사용합니다.", mediaKind: "points", recommendedHiddenUnits: 2,
    points: [
      { x: -0.72, y: -0.72, label: 0 },
      { x: -0.72, y: 0.72, label: 1 },
      { x: 0.72, y: -0.72, label: 1 },
      { x: 0.72, y: 0.72, label: 0 },
    ],
  },
  ...
  webcam: {
    ...
    description: "직접 찍은 가위·바위·보 사진을 모아 새로운 손 모양을 구별해 봅니다.",
    ...
    question: "가위·바위·보 사진을 직접 모아 학습하고, 새로 내민 손도 알아보는지 시험해 볼까요? 배경과 손의 거리를 바꿨을 때도 확인해 봅시다.",
    axes: ["그림 전체", "그림 전체"], classes: ["가위", "바위", "보"], difficulty: "웹캠", sourceNote: "촬영한 사진은 이 브라우저의 메모리에만 보관합니다. 중앙의 정사각형을 14×14 흑백 그림으로 줄여 196칸을 모두 학습합니다. 손뿐 아니라 배경도 입력됩니다. 작은 연습용 모델이므로 실제 손을 항상 알아보지는 못합니다.", mediaKind: "points", recommendedHiddenUnits: 8, points: [],
  },
  custom: {
    title: "빈 화면에서 직접 만들기",
