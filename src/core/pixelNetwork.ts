...
export function pixelActivation(value: number, activation: ActivationName): number {
  if (activation === "relu") return Math.max(0, value);
  if (activation === "sigmoid") return 1 / (1 + Math.exp(-value));
  return Math.tanh(value);
}

function activationDerivative(output: number, activation: ActivationName): number {
  if (activation === "relu") return output > 0 ? 1 : 0;
  if (activation === "sigmoid") return output * (1 - output);
  return 1 - output * output;
}

export function initializePixelModel(inputSize: number, hiddenUnits: number, classCount: number, seed = 31, activation: ActivationName = "tanh"): PixelModel {
  const random = randomSource(seed); const inputScale = Math.sqrt(2 / inputSize); const outputScale = Math.sqrt(2 / hiddenUnits);
  return {
    inputSize, hiddenUnits, classCount, epoch: 0, activation,
...
export function forwardPixels(model: PixelModel, pixels: number[]): PixelForward {
  const hidden = model.inputHidden.map((weights, h) => pixelActivation(weights.reduce((sum, weight, index) => sum + weight * (pixels[index] ?? 0), model.hiddenBias[h] ?? 0), model.activation ?? "tanh"));
  ...
  for (let epoch = 0; epoch < epochs; epoch += 1) {
    ...
    data.forEach((example) => {
      ...
      for (let h = 0; h < next.hiddenUnits; h += 1) {
        const hiddenValue = hidden[h] ?? 0; const delta = (hiddenDelta[h] ?? 0) * activationDerivative(hiddenValue, next.activation ?? "tanh");
