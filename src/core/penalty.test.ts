...
describe('승부차기 사진 선택과 실제 계산',()=>{
  ...
  it('교수용 1뉴런은 네 점 중 하나를 놓치고 2뉴런은 모두 맞힌다',()=>{
    ...
    expect(forward(penaltyTeachingModel(),.5,.5)).toMatchObject({hidden:[.75,0],logit:-.25});
  });
  ...
  it('대표선 움직임은 실제 학습이며 점을 옮기지 않고 골 예상이 커진다',()=>{
    ...
    expect(before.logit).toBeCloseTo(-.8);expect(after.probability).toBeGreaterThan(.5);
